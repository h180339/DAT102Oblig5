...
public class TabellHaug<T extends Comparable<T>> {
	...
	private void reparerOpp() {
		...
		boolean ferdigReparert = false;
		int forelderIndex = (antall) / 2;
		int elementIndex = antall - 1;

		while (!ferdigReparert) {
			if (data[forelderIndex].compareTo(data[elementIndex]) < 0 || elementIndex == 0) {
				ferdigReparert = true;

			} else {
				T forelderVerdi = data[forelderIndex];
				data[forelderIndex] = data[elementIndex];
				data[elementIndex] = forelderVerdi;
			}
			elementIndex = forelderIndex;
			forelderIndex = (elementIndex) / 2;
		}
